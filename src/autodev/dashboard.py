...
def render_dashboard(store: StateStore) -> str:
    ...
    data = metrics(state)
    screenshot_dir = store.directory / "artifacts" / "screenshots"
    screenshots = "".join(
        f'<li><a href="/artifacts/screenshots/{name}">{name}</a></li>'
        for name in ("desktop.png", "narrow.png") if (screenshot_dir / name).is_file()
    ) or "<li>No screenshots yet.</li>"
...
<h2>Runtime</h2><pre>Tasks: {data['tasks_completed']}/{data['tasks_created']}\nRepairs: {data['repairs']}\nReviewer rejects: {data['reviewer_rejects']}\nLLM calls: {data['total_llm_calls']}\nCalls by role: {html.escape(str(data['llm_calls_by_role']))}\nRecoveries: {data['resume_events']}\nLast checkpoint: {html.escape(state.last_checkpoint or 'None')}</pre>
<h2>Designer / visual QA</h2><pre>{html.escape(state.visual_status)}\n{html.escape(chr(10).join(state.visual_issues))}</pre><ul>{screenshots}</ul>
...
def serve_dashboard(store: StateStore, controller: DashboardController, host: str = "127.0.0.1", port: int = 8765) -> None:
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            if path.startswith("/artifacts/screenshots/"):
                name = path.rsplit("/", 1)[-1]
                target = store.directory / "artifacts" / "screenshots" / name
                if name not in {"desktop.png", "narrow.png"} or not target.is_file():
                    self.send_error(404)
                    return
