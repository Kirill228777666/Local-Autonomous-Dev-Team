...
def metrics(state: ProjectState) -> dict[str, object]:
    ...
    return {
        ...
        "dependency_installs": count("ENVIRONMENT", "REPAIRED") + count("ENVIRONMENT", "DEPENDENCY_PROVISIONED"),
        "dependency_install_failures": count("ENVIRONMENT", "DEPENDENCY_PROVISION_FAILED") + sum("Environment limitation:" in entry and "install" in entry.lower() for entry in history),
