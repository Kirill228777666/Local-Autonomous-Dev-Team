...
def metrics(state: ProjectState) -> dict[str, object]:
    ...
    return {
        ...
        "destructive_write_recoveries": sum(event.phase == "TOOL_RECOVERY" and "targeted edit" in event.message.lower() for event in events),
        "destructive_write_recovery_failures": sum("DESTRUCTIVE_WRITE" in entry and "Coder error" in entry for entry in history),
